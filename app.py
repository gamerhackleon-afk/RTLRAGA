...
# --- 6. FUNCIÓN INYECTORA DE ESTILOS (via components.html — SÍ ejecuta JS) ---
# Esta es la ÚNICA técnica en Streamlit que garantiza ejecución de JavaScript.
# st.markdown con <script> NO ejecuta JS — Streamlit lo filtra.
# components.v1.html() corre en un iframe separado que accede al parent via
# window.parent.document para estilizar los botones del documento principal.

def inject_button_styles():
    """
    Genera y ejecuta un bloque JS que busca cada botón por su texto visible
    y aplica los estilos correctos. Corre dentro de un iframe de Streamlit
    (components.v1.html) con acceso al parent document.
    """

    # ── Tabla de estilos: (label, bg, fg, activo, borde_act, sombra_act, grayscale_inactivo)
    # Los botones de DIAS INV y NEGATIVOS pueden pertenecer a diferentes retailers,
    # por lo que el color depende del retailer activo actualmente.
    _dias_bg     = "#1B5E20" if act == "SORIANA" else ("#00695C" if act == "WALMART" else "#4E342E")
    _dias_shadow = "rgba(27,94,32,0.85)" if act == "SORIANA" else ("rgba(0,105,92,0.85)" if act == "WALMART" else "rgba(78,52,46,0.85)")
    _dias_active = s_dias_inv or w_dias_inv or c_dias_inv

    _neg_bg     = "#E65100" if act == "WALMART" else "#B71C1C"
    _neg_shadow = "rgba(230,81,0,0.85)" if act == "WALMART" else "rgba(183,28,28,0.85)"
    _neg_active = w_neg or c_neg_zero

    # Ranking: los 3 retailers comparten el mismo estilo de ranking
    ...
    STYLES = [
        # label,              bg,        fg,        activo,       borde_act,  sombra_act,                grayscale
        # ── Navegación principal ─────────────────────────────────────────────
        ("SORIANA",  "linear-gradient(135deg,#D32F2F,#B71C1C)", "#ffffff",
         act=="SORIANA",  "#ffffff", "rgba(0,0,0,0.3)", False),
        ("WALMART",  "linear-gradient(135deg,#0071DC,#005BB5)", "#ffffff",
         act=="WALMART",  "#ffffff", "rgba(0,0,0,0.3)", False),
        ("CHEDRAUI", "linear-gradient(135deg,#FF6600,#E65100)", "#ffffff",
         act=="CHEDRAUI", "#ffffff", "rgba(0,0,0,0.3)", False),
        # ── SORIANA acción ───────────────────────────────────────────────────
        ("🔴 INV SIN VENTA", "#D32F2F","#ffffff", s_rojo,      "#ffffff","rgba(211,47,47,0.85)",  False),
        ("📋 DIAS X PROD",   "#0D47A1","#ffffff", s_dias_prod, "#ffffff","rgba(13,71,161,0.85)",  False),
        # ── WALMART acción ───────────────────────────────────────────────────
        ("🔴 SIN VTA 4SEM",  "#0071DC","#ffffff", w_4w,        "#ffffff","rgba(0,113,220,0.85)",  False),
        ("📋 DIAS X PROD",   "#4A148C","#ffffff", w_dias_prod, "#ffffff","rgba(74,20,140,0.85)",  False),
        # ── Compartidos DIAS INV y NEGATIVOS (color según retailer activo) ──
        ("📅 DIAS INV",  _dias_bg, "#ffffff", _dias_active, "#ffffff", _dias_shadow, False),
        ("📉 NEGATIVOS", _neg_bg,  "#ffffff", _neg_active,  "#ffffff", _neg_shadow,  False),
        # ── RANKING — igual para los 3 retailers ────────────────────────────
        ("📊 GENERAL",  "#FFFFFF","#5AB027", _gen_active, "#D4D4D4","rgba(46,125,50,0.70)", False),
        ("🍝 PASTAS",   "#DBBB35","#075E12", _pas_active, "#D4D4D4","rgba(46,125,50,0.70)", True),
        ("🫒 OLIVAS",   "#4E5C02","#ffffff", _oli_active, "#D4D4D4","rgba(46,125,50,0.70)", True),
        ("🍃 NUTRIOLI", "#2E7D32","#FFD700", _nut_active, "#D4D4D4","rgba(46,125,50,0.70)", True),
        ("🏆 NUTRIOLI", "#2E7D32","#FFD700", _nut_active, "#D4D4D4","rgba(46,125,50,0.70)", True),
    ]

    # Construir las reglas JS
    js_cases = []
    for (label, bg, fg, active, border_act, shadow_act, gs) in STYLES:
        gs_val = "grayscale(25%)" if gs else "none"
        ...
        if active:
            ...
            # Botones NAV activos son más grandes
            ...
            props = (
                ...
                f"b.style.setProperty('font-weight','800','important');"
                f"b.style.setProperty('border-radius','8px','important');"
                f"b.style.setProperty('border','1px solid rgba(0,0,0,0.15)','important');"
...
# --- 7. CSS BASE (solo layout, sin tocar botones) ---
st.markdown(f"""
<style>
...
# ── Inyectar estilos JS después de renderizar los botones de nav ──────────────
...
def view_walmart(df_w):
    ...
    if df_w is not None:
        ...
        conditions_w = [is_borges, desc_w.str.contains("NUTRIOLI",na=False)&desc_w.str.contains("946",na=False), desc_w.str.contains("SABROSANO",na=False), desc_w.str.contains("GRANTRADICION",na=False), desc_w.str.contains("BALSAMICO",na=False), (desc_w.str.contains("OLISPRAY|OLICOCINA|OLIDENUTEV|ACEITEOLIDEOLIVA|OLIDENUT",na=False))&~desc_w.str.contains("BALSAMICO",na=False), desc_w.str.contains("NUTRIOLI",na=False)&desc_w.str.contains("SPAGUETTI|FIDEO|CODO|PASTA",na=False), desc_w.str.contains("NUTRIOLI",na=False)]
        ...
        with r1: st.button("📊 GENERAL",  key="w_rk_gen", on_click=set_rank, args=('tiendas',),  use_container_width=True, type="primary" if w_rank_tiendas else "secondary")
        with r2: st.button("🍝 PASTAS",   key="w_rk_pas", on_click=set_rank, args=('pastas',),   use_container_width=True, type="primary" if w_rank_pastas  else "secondary")
        with r3: st.button("🫒 OLIVAS",   key="w_rk_oli", on_click=set_rank, args=('olivas',),   use_container_width=True, type="primary" if w_rank_olivas  else "secondary")
        with r4: st.button("🏆 NUTRIOLI", key="w_rk_nut", on_click=set_rank, args=('nutrioli',), use_container_width=True, type="primary" if w_nutri_top10  else "secondary")
...
def view_chedraui(df_c):
    ...
    if df_c is not None:
        ...
        if st.session_state.c_dias_inv:
            ...
            conditions_c = [desc_c.str.contains("BALSAMICO",na=False), desc_c.str.contains("SABROSANO",na=False), desc_c.str.contains("GRANTRADICION",na=False), desc_c.str.contains("MISAZON|MISAZÓN",na=False), desc_c.str.contains("AVE",na=False)&desc_c.str.contains("SOYA-CANOLA|AEROSOL",na=False), desc_c.str.contains("NUTRIOLI",na=False)&desc_c.str.contains("FUSILLI|SPAGUETTI|FIDEO|CODO",na=False), desc_c.str.contains("OLI",na=False)&desc_c.str.contains("OLIVA|EV|AEROSOL",na=False), desc_c.str.contains("NUTRIOLI",na=False)&desc_c.str.contains("400ML|850ML",na=False)&~desc_c.str.contains("PROTECT|DEFENSAS",na=False), desc_c.str.contains("NUTRIOLI",na=False)]
...
# Inyectar estilos de nuevo al final (cubre todos los botones renderizados en las vistas)
inject_button_styles()
